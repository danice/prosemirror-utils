...
import { Node as ProsemirrorNode, Fragment, NodeType, Mark } from 'prosemirror-model';
import { findParentNodeOfType, findPositionOfNodeBefore } from 'prosemirror-utils-bangle/src/selection';
import {
  cloneTr,
  isNodeSelection,
  replaceNodeAtPos,
  ...
  canInsert,
  isEmptyParagraph
} from 'prosemirror-utils-bangle/src/helpers';
...
// );
// ```
export function replaceParentNodeOfType(nodeType: NodeType | NodeType[], content: ProsemirrorNode): (tr: Transaction) => Transaction
{ 
  return (tr) => {
...
// );
// ```
export function replaceSelectedNode(content: ProsemirrorNode): (tr: Transaction) => Transaction
{
  return (tr) => {
    if (isNodeSelection(tr.selection)) {
...
// );
// ```
export function safeInsert(content: ProsemirrorNode | Fragment, position?: number, tryToReplace?: boolean): (tr: Transaction) => Transaction
{ 
  return (tr) => {
    ...
    if (isNodeSelection(tr.selection) && tryToReplace) {
      const oldTr = tr;
      tr = replaceSelectedNode((content as ProsemirrorNode))(tr);
      if (oldTr !== tr) {
        return tr;
      }
    }
    ...
    if (isEmptyParagraph(parent)) {
      const oldTr = tr;
      tr = replaceParentNodeOfType(parent.type, (content as ProsemirrorNode))(tr);
      if (oldTr !== tr) {
